...
from server.defaults import API_TOKEN
...
def query_noaa_api():
    ENDPOINT = NOAA_ENDPOINT + "data?" + str(request.query_string)
    try:
...
def verify_dataset_categories():
    ENDPOINT = NOAA_ENDPOINT + "datacategories?" + str(request.query_string)
    try:
        api_response = requests.get(ENDPOINT, headers=headers)
        api_response.raise_for_status()
        return jsonify(api_response.json())
...
def verify_dataset_location():
    ENDPOINT = NOAA_ENDPOINT + "locations?" + str(request.query_string)
    try:
        api_response = requests.get(ENDPOINT, headers=headers)
        api_response.raise_for_status()
        return jsonify(api_response.json())
...
def get_datatypes_by_dataset():
    ENDPOINT = NOAA_ENDPOINT + "datatypes?"
    print('REQ: ', request.args)
    try:
        api_response = requests.get(ENDPOINT, headers=headers, params=request.args)
        api_response.raise_for_status()
        return jsonify(api_response.json())
    except requests.exceptions.RequestException as e:
        return jsonify({"ApiError": {"type": "InvalidParameters","message": "Invalid dataset id."}})
...
def get_stations_by_coords():
    ENDPOINT = NOAA_ENDPOINT + "stations?"
    try:
        api_response = requests.get(ENDPOINT, headers=headers, params=request.args)
        api_response.raise_for_status()
        return jsonify(api_response.json())
